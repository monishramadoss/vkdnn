                                                                        ...
                                                                        offset_(nullptr)
{
    ...
    for (uint32_t i = 1; i < n_dims_; ++i)
        stride_[i] = stride_[i - 1] * static_cast<size_t>(shape_[static_cast<size_t>(i) - 1]);
                                ...
                                n_offsets_(v.n_offsets_), offset_(new offset*[n_offsets_])
{
    ...
    if (n_offsets_ != 0)
        std::memcpy(offset_, v.offset_, n_offsets_ * sizeof(offset*));
    std::memcpy(size_, v.size_, static_cast<size_t>(n_dims_ + 1) * sizeof(size_t));
}
...
tensor::tensor(std::vector<uint32_t>& shape, const DTYPE type) : view_(shape.data(), static_cast<uint32_t>(shape.size()), get_data_size(type)),
                                                                    data_(new vk_block* [2]), parent_(nullptr), d_type_(type),
...
tensor::tensor(const std::vector<uint32_t>& shape, const DTYPE type) :	view_(shape.data(), static_cast<uint32_t>(shape.size()), get_data_size(type)),
                                                                        data_(new vk_block*[2]), parent_(nullptr), d_type_(type),
                                                                        name_("tensor_" + std::to_string(++k_tensor_count)),
                                                                        cspec_({ })
...
tensor::tensor(tensor* ptr, view v) : view_(std::move(v)), data_(nullptr), parent_(ptr), d_type_(ptr->d_type_),
                                      name_("tensor_" + std::to_string(++k_tensor_count)), cspec_({ })
{
}
...
tensor::tensor(tensor&& t) noexcept : view_(std::move(t.view_)), data_(t.data_), parent_(t.parent_), d_type_(t.d_type_),
                                      name_(std::move(t.name_)), cspec_(std::move(t.cspec_))
{
    t.data_ = nullptr;
}
