...
public:
    runtime& create() {
        ...
        if (device_count_ != 0 && result == VK_SUCCESS)
        {
            ...
            for (uint32_t d = 0; d < device_count_; ++d)
                devices_[d] = new device(d, device_count_, instance_, physical_devices_[d]);
        }
        if(result != VK_SUCCESS)
