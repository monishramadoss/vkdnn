...
inline std::vector<uint32_t> compile(const std::string& shader_entry, const std::string& source, char* filename)
{
    ...
    if (tmp_file == 0)
        throw std::runtime_error("cannot create the tmp file to store compile results");
    ...
    const auto cmd_str = std::string(
        "glslangValidator -V --target-env vulkan1.2 --quiet " + std::string(tmp_filename_in) + " --entry-point " + shader_entry +
...
public:
    ...
    job(std::string kernel_name) : ci_({}), kernel_type_(std::move(kernel_name))
    {
